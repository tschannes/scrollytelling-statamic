...
if (window.Statamic && window.Statamic.$conditions) {
    ...
    const tileParallaxCache = {};

    window.Statamic.$conditions.add('isStoryParallaxActive', (args) => {
        const { target, values, store, root } = args || {};
        const logMsg = (msg) => {
            console.log('[isStoryParallaxActive]', msg);
            if (window.ScrollytellingDebug) {
                window.ScrollytellingDebug(msg);
            }
        };

        logMsg(`Evaluating. target=${target}, values=${JSON.stringify(values)}`);

        // 1. Search Vue components attached to DOM (works across Vue 2 and Vue 3)
        try {
            const elements = document.querySelectorAll('.publish-container, .publish-form, [data-field-name="parallax"], *');
            for (const el of elements) {
                const vm = el.__vue__ || el.__vueParentComponent || el.__vue_app__?._instance;
                if (vm) {
                    const containers = [
                        vm,
                        vm.values,
                        ...
                        vm.props?.values,
                        vm.setupState,
                        vm.setupState?.values,
                        vm.ctx,
                        vm.ctx?.values
                    ];
                    for (const container of containers) {
                        if (container && container.parallax !== undefined) {
                            const isParallax = !!container.parallax;
                            logMsg(`Found parallax=${isParallax} in Vue component context`);
                            return isParallax;
                        }
                    }
                }
            ...
            logMsg(`Error searching Vue components: ${e.message}`);
        }

        // 2. Search in global Statamic Vuex store/state if Vuex is used
        try {
            if (window.Statamic && window.Statamic.$store && window.Statamic.$store.state && window.Statamic.$store.state.publish) {
                const publish = window.Statamic.$store.state.publish;
                for (const ns in publish) {
                    if (publish[ns] && publish[ns].values && publish[ns].values.parallax !== undefined) {
                        const isParallax = !!publish[ns].values.parallax;
                        logMsg(`Found parallax=${isParallax} in Vuex store under namespace '${ns}'`);
                        return isParallax;
                    }
                }
            }
        } catch (e) {
            logMsg(`Error checking Vuex store: ${e.message}`);
        }

        // 3. Search DOM directly for toggle fields or inputs
        try {
            const container = document.querySelector('#field_parallax');
            if (container) {
                const btnChecked = container.getAttribute('aria-checked') || container.ariaChecked;
                logMsg(`Found parallax DOM element. aria-checked=${btnChecked}`);
                return btnChecked === 'true';
            }
        } catch (e) {
            logMsg(`Error checking DOM: ${e.message}`);
        }

        // 4. Check local form values (if editing story directly)
        if (values && values.parallax !== undefined) {
            const isParallax = !!values.parallax;
            logMsg(`Found parallax=${isParallax} in local values`);
            return isParallax;
        }

        // 5. Standalone Tile editing fallback: Fetch from backend API
        let tileId = null;
        if (values && values.id) {
            tileId = values.id;
        }
        if (!tileId) {
            ...
            if (collectionsIdx !== -1 && entriesIdx === collectionsIdx + 2 && pathParts[collectionsIdx + 1] === 'tiles') {
                tileId = pathParts[entriesIdx + 1];
            }
        }
        if (!tileId && window.Statamic && window.Statamic.$store && window.Statamic.$store.state && window.Statamic.$store.state.publish) {
            const publish = window.Statamic.$store.state.publish;
            for (const ns in publish) {
                if (publish[ns] && publish[ns].id) {
                    tileId = publish[ns].id;
                    break;
                }
            }
        }
        ...
        if (tileId) {
            if (tileParallaxCache[tileId] !== undefined) {
                logMsg(`Cache hit for tile ${tileId}: ${tileParallaxCache[tileId]}`);
                return tileParallaxCache[tileId];
            }

            try {
                logMsg(`Making sync XHR request to check parallax for tile: ${tileId}`);
                const xhr = new XMLHttpRequest();
                xhr.open('GET', `/_scrollytelling/api/story-parallax?tile=${tileId}`, false);
                xhr.send();
                if (xhr.status === 200) {
                    const res = JSON.parse(xhr.responseText);
                    const isParallax = !!res.parallax;
                    tileParallaxCache[tileId] = isParallax;
                    logMsg(`API response: tile ${tileId} has parallax=${isParallax}`);
                    return isParallax;
                }
            } catch (e) {
                logMsg(`Error making sync API call: ${e.message}`);
            }
        }

        logMsg('No parallax field found, defaulting to false');
        return false;
    });
}
